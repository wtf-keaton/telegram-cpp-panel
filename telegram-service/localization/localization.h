/*
*
* I do not want to separate this file to .cpp and .h ones, because this one is not big as it might be.
* Need some examples\tests. Perhaps, I may do it later.
* Advice:
* Maybe we should use std::flat_map since C++23, if there won't be a lot of data(less than about 64).
* Add hash for string literars.
*/
...
namespace __details {
    ...
    constexpr std::string source_location_format(std::source_location lc) {
        return format("filename'", lc.file_name(),
        "' in function='", lc.function_name(),
        "' at line ", lc.line(),
         " column ", lc.column());   
    }
}
            ...
            throw std::runtime_error{
                __details::format(
                    "in [",
                     ...
                     "] cannot add second value as value='",
                     value,
                     "' for synonym='",
                     synonim,
                     '\'')};
