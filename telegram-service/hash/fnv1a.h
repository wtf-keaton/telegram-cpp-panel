...
namespace fnv1a
{
	...
	template < typename _ty > constexpr hash_t hash( const _ty str ) { return std::is_const_v< _ty > ? fnv1a::ct( str ) : fnv1a::rt( str ); }
}

#define HASH( s ) fnv1a::hash( s )
#define HASH_VAL( val ) ~static_cast< unsigned long long >( 1l * ( fnv1a::fnv_basis ^ val ) )

inline hash_t operator""_h( const char* c, size_t )
{
	return HASH( c );
}
